  ...
  checkout: async function (form) {
    ...
    try {
      ...
      if (res.orderId) {
        ...
        window.location.replace("success.html"); // this way user can't go back, since that would be buggy
      } else {
        console.log("Order failed");
        console.log(res);
      }
    } catch (error) {
      console.log(`There was an error with your checkout.`, error.message);
      alert(
        `There was an error with your checkout. ${JSON.stringify(error.message)}`,
      );
    }
  },
};
